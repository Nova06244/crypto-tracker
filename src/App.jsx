...
if (rows.length === 0) { setPreview({ rows: [], matched: 0, errors: [“Aucune ligne trouvée”] }); return; }
...
const entries = rows.map((r, idx) => {
  ...
  if (!asset) { errors.push(`Ligne ${idx+2}: actif "${ticker||name}" non trouvé`); return null; }
  ...
  if (isNaN(year)) { errors.push(`Ligne ${idx+2}: année invalide`); return null; }
  matched++;
  return {
...
Ticker;Mois;Année;Investissement;PrixAchat;Quantité;StopLoss;TakeProfit<br/>
SOL;2;2025;100;150.5;0.66;120;300<br/>
BTC;2;2025;200;92000;0.002;80000;150000
</div>
<div>Séparateur : <b>;</b> ou <b>,</b> (détection auto). Les colonnes Ticker/Actif et Mois/Année sont obligatoires.</div>
</div>
...
```
      ...
      <div>
        ...
        <textarea value={csvText} onChange={e=>{setCsvText(e.target.value);if(e.target.value.trim())previewCSV(e.target.value);else setPreview(null);}}
          placeholder="Ticker;Mois;Année;Investissement;PrixAchat;Quantité" rows={5}
    ...
    {activeView==="dashboard"?(
      <>
        <div style={{background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.06)",borderRadius:16,padding:"20px 24px",marginBottom:28}}>
          <div style={{fontSize:11,fontWeight:600,color:"rgba(255,255,255,0.4)",letterSpacing:2,marginBottom:16,display:"flex",alignItems:"center",gap:8}}><Icons.Chart/> RÉPARTITION MENSUELLE {year}</div>
