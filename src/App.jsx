    ...
    <div style={{ position: "fixed", inset: 0, zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
      ...
      >
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
          ...
          <button onClick={onClose} style={{ background: "none", border: "none", color: "rgba(255,255,255,0.4)", fontSize: 24, cursor: "pointer", lineHeight: 1 }}>
            ×
          </button>
        </div>
        {children}
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="NOUVEL INVESTISSEMENT">
      <div style={{ display: "grid", gap: 14 }}>
        ...
        <div>
          <label style={labelStyle}>Quantité (jetons)</label>
      ...
      <div style={{ display: "grid", gap: 14 }}>
        <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: 12 }}>
          <div>
            ...
            <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Avalanche" style={inputStyle} />
          </div>
          <div>
            <label style={labelStyle}>Ticker</label>
            <input value={ticker} onChange={(e) => setTicker(e.target.value)} placeholder="AVAX" style={inputStyle} />
          </div>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
          <div>
            ...
            <input value={objective} onChange={(e) => setObjective(e.target.value)} placeholder="X5" style={inputStyle} />
          </div>
          <div>
            ...
            <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
              <input
                ...
                style={{ width: 40, height: 38, border: "none", borderRadius: 8, cursor: "pointer", background: "transparent" }}
              />
...
function ImportCSVModal({ isOpen, onClose, assets, onImport }) {
  ...
  const previewCSV = (text) => {
    ...
    if (rows.length === 0) {
      setPreview({ rows: [], matched: 0, errors: ["Aucune ligne trouvée"], headers: [], totalRows: 0 });
      return;
    }
    ...
    const entries = rows
      .map((r, idx) => {
        ...
        if (!asset) {
          errors.push(`Ligne ${idx + 2}: actif "${ticker || name}" non trouvé`);
          return null;
        }

        const month = normalizeMonth(getField(r, ["Mois", "mois", "Month", "month"]));
        const year = parseInt(getField(r, ["Année", "Annee", "Year", "year", "Annee"]), 10);

        if (month === null) {
          errors.push(`Ligne ${idx + 2}: mois invalide`);
          return null;
        }
        if (Number.isNaN(year)) {
          errors.push(`Ligne ${idx + 2}: année invalide`);
          return null;
        }
        ...
        return {
          ...
          buyPrice: toNum(getField(r, ["PrixAchat", "Prix d'achat", "buyPrice", "BuyPrice", "Prix"])),
          quantity: toNum(getField(r, ["Quantité", "Quantite", "quantity", "Quantity", "Qty"])),
    ...
    setPreview({ rows: entries, matched, errors, headers, totalRows: rows.length });
  };
    ...
    <Modal isOpen={isOpen} onClose={handleClose} title="IMPORTER UN CSV">
      {result ? (
        <div style={{ textAlign: "center", padding: "20px 0" }}>
          ...
          <div style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 24, color: "#2ed573", marginBottom: 8 }}>{result.count} INVESTISSEMENTS IMPORTÉS</div>
          <div style={{ fontSize: 13, color: "rgba(255,255,255,0.5)" }}>Les données ont été ajoutées à ton portefeuille</div>
          <button
            onClick={handleClose}
            style={{ marginTop: 20, padding: "10px 30px", background: "rgba(255,255,255,0.1)", border: "none", borderRadius: 8, color: "#fff", cursor: "pointer", fontSize: 13, fontWeight: 600 }}
          >
            Fermer
          </button>
        </div>
        ...
        <div style={{ display: "grid", gap: 16 }}>
          ...
          <div style={{ background: "rgba(255,255,255,0.03)", borderRadius: 10, padding: 14, fontSize: 12, color: "rgba(255,255,255,0.5)", lineHeight: 1.6 }}>
            ...
            >
              ...
              <br />
              SOL;2;2025;100;150.5;0.66;120;300
              <br />
              BTC;2;2025;200;92000;0.002;80000;150000
            </div>
            <div>
              Séparateur : <b>;</b> ou <b>,</b> (détection auto). Les colonnes Ticker/Actif et Mois/Année sont obligatoires.
            </div>
          </div>
          ...
          <div>
            ...
            >
              <Icons.Upload /> Choisir un fichier CSV
            </button>
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              onChange={(e) => {
                ...
                setCsvText(t);
                if (t.trim()) previewCSV(t);
                else setPreview(null);
              }}
              placeholder="Ticker;Mois;Année;Investissement;PrixAchat;Quantité"
              rows={5}
          ...
          <button
            ...
            style={{
              ...
              border: "none",
              borderRadius: 10,
              color: preview?.rows?.length ? "#fff" : "rgba(255,255,255,0.2)",
              fontSize: 14,
              fontWeight: 700,
              letterSpacing: 1.5,
              cursor: preview?.rows?.length ? "pointer" : "not-allowed",
              fontFamily: "'DM Sans',sans-serif",
              textTransform: "uppercase",
            }}
          >
...
function AssetCard({ asset, year, onDelete }) {
  ...
  return (
    <div
      style={{ background: "rgba(255,255,255,0.03)", border: `1px solid ${asset.color}22`, borderRadius: 14, overflow: "hidden", transition: "all 0.3s" }}
      ...
      >
        ...
        <div style={{ flex: 1 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            {asset.objective && (
              <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: 1, padding: "2px 8px", borderRadius: 6, background: asset.color + "22", color: asset.color }}>
                {asset.objective}
              </span>
            )}
          </div>
          <div style={{ fontSize: 12, color: "rgba(255,255,255,0.4)", marginTop: 2 }}>
            {asset.entriesCum.length} invest. • {fmt(totalInv)}
          </div>
        </div>
        ...
        <div style={{ borderTop: "1px solid rgba(255,255,255,0.06)", padding: "16px 20px", animation: "fadeIn 0.3s" }}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 10, marginBottom: 16 }}>
            {[
              ...
              { label: "Quantité", value: totalQty > 0 ? totalQty.toLocaleString("fr-FR") : "—" },
          ...
          {asset.entriesCum.length === 0 ? (
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              {asset.entriesCum.map((e) => (
                <div
                  key={e.id}
                  style={{ display: "grid", gridTemplateColumns: "90px 1fr 1fr 1fr 36px", gap: 8, alignItems: "center", padding: "8px 12px", borderRadius: 8, background: "rgba(255,255,255,0.02)", fontSize: 13 }}
                >
                  ...
                  <button
                    ...
                    onMouseOver={(ev) => {
                      ev.currentTarget.style.color = "#ff4757";
                      ev.currentTarget.style.background = "rgba(255,71,87,0.1)";
                    }}
                    onMouseOut={(ev) => {
                      ev.currentTarget.style.color = "rgba(255,255,255,0.2)";
...
export default function CryptoTracker() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (saved) {
        ...
        showToast(`${saved.entries?.length || 0} investissements chargés`, "success");
      }
      setLoaded(true);
    })();
  }, []);
  ...
  // ✅ COMPUTED: cumulative data up to selected year
  const assetData = useMemo(() => {
    return assets
      .map((asset) => {
        ...
        const entriesYear = entries.filter((e) => e.assetId === asset.id && e.year === year).sort((a, b) => a.month - b.month);
        ...
        const avgBuyCum =
          entriesCum.length > 0 ? entriesCum.reduce((s, e) => s + (e.buyPrice || 0) * (e.investment || 0), 0) / (totalCum || 1) : 0;
  ...
  const handleAddAsset = (data) => {
    setAssets((prev) => [...prev, { id: nextId, ...data }]);
    setNextId((n) => n + 1);
  };
  const handleImportCSV = (newEntries) => {
    setEntries((prev) => [...prev, ...newEntries]);
    showToast(`${newEntries.length} investissements importés !`, "success");
  };
      ...
      {toast && (
        <div
          style={{
            ...
            fontSize: 13,
            fontWeight: 600,
            background: toast.type === "success" ? "#2ed573" : "#00d2ff",
            color: "#000",
      ...
      <header style={{ padding: "24px 32px 20px", borderBottom: "1px solid rgba(255,255,255,0.06)", background: "rgba(13,13,26,0.8)", backdropFilter: "blur(20px)", position: "sticky", top: 0, zIndex: 100 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 14, flexWrap: "wrap" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
            <div style={{ width: 42, height: 42, borderRadius: 12, background: "linear-gradient(135deg,#00d2ff,#3a7bd5)", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 4px 20px rgba(0,210,255,0.2)" }}>
              <Icons.Wallet />
            </div>
            <div>
              <h1
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 4, background: "rgba(255,255,255,0.05)", borderRadius: 10, padding: "6px 4px" }}>
              <button onClick={() => setYear((y) => y - 1)} style={{ background: "none", border: "none", cursor: "pointer", color: "rgba(255,255,255,0.4)", padding: "4px 8px", borderRadius: 6, display: "flex", alignItems: "center" }}>
              ...
              {[{ k: "dashboard", l: "Dashboard" }, { k: "table", l: "Tableau" }].map((v) => (
                <button
                  ...
                  style={{
                    ...
                    letterSpacing: 0.5,
                    fontFamily: "'DM Sans',sans-serif",
                    background: activeView === v.k ? "rgba(0,210,255,0.15)" : "transparent",
                    color: activeView === v.k ? "#00d2ff" : "rgba(255,255,255,0.4)",
                    transition: "all 0.2s",
                  }}
                >
                  {v.l}
            ...
            <button onClick={() => setShowImport(true)} style={{ padding: "8px 14px", border: "1px solid rgba(46,213,115,0.3)", borderRadius: 10, background: "rgba(46,213,115,0.08)", color: "#2ed573", cursor: "pointer", fontSize: 12, fontWeight: 600, display: "flex", alignItems: "center", gap: 6, fontFamily: "'DM Sans',sans-serif" }}>
              <Icons.Upload /> CSV
            </button>
            <button onClick={() => setShowAddAsset(true)} style={{ padding: "8px 14px", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 10, background: "rgba(255,255,255,0.04)", color: "rgba(255,255,255,0.6)", cursor: "pointer", fontSize: 12, fontWeight: 600, display: "flex", alignItems: "center", gap: 6, fontFamily: "'DM Sans',sans-serif" }}>
              <Icons.Plus /> Actif
            </button>
            <button onClick={() => setShowAddInv(true)} style={{ padding: "8px 18px", background: "linear-gradient(135deg,#00d2ff,#3a7bd5)", border: "none", borderRadius: 10, color: "#fff", cursor: "pointer", fontSize: 12, fontWeight: 700, letterSpacing: 1, display: "flex", alignItems: "center", gap: 6, fontFamily: "'DM Sans',sans-serif", boxShadow: "0 4px 16px rgba(0,210,255,0.2)" }}>
              <Icons.Plus /> Investir
            </button>
          </div>
        </div>
        ...
        {activeView === "dashboard" ? (
          <>
            <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 16, padding: "20px 24px", marginBottom: 28 }}>
              <div style={{ fontSize: 11, fontWeight: 600, color: "rgba(255,255,255,0.4)", letterSpacing: 2, marginBottom: 16, display: "flex", alignItems: "center", gap: 8 }}>
                <Icons.Chart /> RÉPARTITION MENSUELLE {year}
              </div>
                ...
                <tbody>
                  {assetData.map((asset) => (
                    <tr key={asset.id} style={{ borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                      ...
                      <td style={{ padding: "12px 16px", textAlign: "center", fontWeight: 700, color: asset.totalCum > 0 ? "#fff" : "rgba(255,255,255,0.12)", background: "rgba(0,210,255,0.04)", fontFamily: "'Bebas Neue',sans-serif", fontSize: 15 }}>
                        {asset.totalCum > 0 ? fmt(asset.totalCum) : "—"}
                      </td>
                    </tr>
                  ))}
                  ...
                  <tr style={{ borderTop: "2px solid rgba(0,210,255,0.2)", background: "rgba(0,210,255,0.04)" }}>
                    ...
                    {totalMonthly.map((val, i) => (
                      <td key={i} style={{ padding: "14px 12px", textAlign: "center", fontWeight: 700, color: val > 0 ? "#00d2ff" : "rgba(255,255,255,0.12)", fontSize: 12 }}>
                        {val > 0 ? fmt(val) : "—"}
                      </td>
                    ))}
