...
const ALPHAVANTAGE_KEY = "O72E6UJNVWBFZJGV";
...
export default function CryptoTracker() {
  ...
  // ---- UNIQUE fetchStockPrices : prix USD converti en EUR via Alpha Vantage ----
  async function fetchStockPrices(tickers) {
    const results = {};
    // Etape 1 : taux de change USD -> EUR (OBLIGATOIRE avant tout)
    let usdToEur = null;
    try {
      const fxUrl = "https://www.alphavantage.co/query?function=CURRENCY_EXCHANGE_RATE&from_currency=USD&to_currency=EUR&apikey=" + ALPHAVANTAGE_KEY;
      const fxRes = await fetch(fxUrl);
      const fxData = await fxRes.json();
      const rateStr = fxData?.["Realtime Currency Exchange Rate"]?.["5. Exchange Rate"];
      const parsed = parseFloat(rateStr);
      usdToEur = (!isNaN(parsed) && parsed > 0) ? parsed : null;
    ...
    // Etape 2 : prix en USD puis conversion
    await Promise.all(tickers.map(async (ticker) => {
      try {
        const url = "https://www.alphavantage.co/query?function=GLOBAL_QUOTE&symbol=" + encodeURIComponent(ticker) + "&apikey=" + ALPHAVANTAGE_KEY;
      ...
      } catch (e) { console.warn("Stock error:", ticker, e); }
    }));
    return results;
  }
