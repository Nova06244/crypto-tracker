...
export default function CryptoTracker() {
    ...
    await Promise.all(tickers.map(async (ticker) => {
      try {
        ...
        if (!isNaN(priceUsd) && priceUsd > 0) {
          ...
          results[ticker] = { usd: priceUsd, eur: priceEur };
  ...
  async function fetchPrices(assetList) {
    ...
    const debugInfo = Object.entries(merged).map(([t, v]) => t + "=" + (v?.eur != null ? v.eur.toFixed(4) + "EUR" : "N/A")).join(" | ");
    const stockInfo = stockTickers.length > 0 ? " | STOCKS:" + JSON.stringify(stockPrices) : " | pas de stocks detectes";
