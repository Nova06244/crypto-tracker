...
const COINGECKO_IDS = {
  ...
  SEI: "sei-network", XLM: "stellar", BGB: "bitget-token",
  ONDO: "ondo-finance", SPX: "spx6900", VELO: "velo",
};
...
function SvgPlus() { return <svg width="16" height="16" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="2"><line x1="8" y1="2" x2="8" y2="14" /><line x1="2" y1="8" x2="14" y2="8" /></svg>; }
function SvgTrash() { return <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="3 6 5 6 21 6" /><path d="M19 6v14a2 2 0 01-2 2H7a2 2 0 01-2-2V6m3 0V4a2 2 0 012-2h4a2 2 0 012 2v2" /></svg>; }
function SvgLeft() { return <svg width="16" height="16" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="2" style={{ transform: "rotate(180deg)" }}><polyline points="6 3 11 8 6 13" /></svg>; }
function SvgRight() { return <svg width="16" height="16" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="6 3 11 8 6 13" /></svg>; }
function SvgChart() { return <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="18" y1="20" x2="18" y2="10" /><line x1="12" y1="20" x2="12" y2="4" /><line x1="6" y1="20" x2="6" y2="14" /></svg>; }
function SvgWallet() { return <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M21 12V7H5a2 2 0 010-4h14v4" /><path d="M3 5v14a2 2 0 002 2h16v-5" /><path d="M18 12a2 2 0 100 4h4v-4h-4z" /></svg>; }
function SvgHistory() { return <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="12 8 12 12 14 14" /><path d="M3.05 11a9 9 0 1 0 .5-4M3 3v5h5" /></svg>; }
function SvgTrend() { return <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="22 7 13.5 15.5 8.5 10.5 2 17" /><polyline points="16 7 22 7 22 13" /></svg>; }
function SvgRefresh() { return <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="23 4 23 10 17 10" /><path d="M20.49 15a9 9 0 1 1-2.12-9.36L23 10" /></svg>; }
function SvgSpinner() { return <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" style={{ animation: "spin 1s linear infinite", display: "inline" }}><circle cx="12" cy="12" r="10" strokeOpacity="0.2" /><path d="M12 2a10 10 0 0 1 10 10" /></svg>; }
function SvgDash() { return <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="3" width="7" height="7" /><rect x="14" y="3" width="7" height="7" /><rect x="14" y="14" width="7" height="7" /><rect x="3" y="14" width="7" height="7" /></svg>; }
...
function InvestForm({ isOpen, onClose, assets, year, onAdd }) {
  ...
  async function handleSubmit() {
    ...
    setSaving(true);
    const { data, error } = await supabase.from("investments").insert({ asset_id: parseInt(assetId, 10), month, year: entryYear, investment: parseFloat(investment) || 0, buy_price: parseFloat(buyPrice) || 0, quantity: parseFloat(quantity) || 0, stop_loss: 0, take_profit: 0, type: "invest" }).select().single();
    setSaving(false);
    if (!error && data) { onAdd({ id: data.id, assetId: data.asset_id, month: data.month, year: data.year, investment: parseFloat(data.investment), buyPrice: parseFloat(data.buy_price), quantity: parseFloat(data.quantity), type: data.type }); setAssetId(""); setInvestment(""); setBuyPrice(""); setQuantity(""); onClose(); }
  }
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="NOUVEL INVESTISSEMENT">
      <div style={{ display: "grid", gap: 14 }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
          <div><label style={labelStyle}>Actif</label><select value={assetId} onChange={e => setAssetId(e.target.value)} style={{ ...inputStyle, cursor: "pointer" }}><option value="">Choisir...</option>{assets.map(a => <option key={a.id} value={a.id} style={{ background: "#1a1a2e" }}>{a.name} ({a.ticker})</option>)}</select></div>
          <div><label style={labelStyle}>Mois</label><select value={month} onChange={e => setMonth(parseInt(e.target.value, 10))} style={{ ...inputStyle, cursor: "pointer" }}>{MONTHS.map((m, i) => <option key={i} value={i} style={{ background: "#1a1a2e" }}>{m}</option>)}</select></div>
          <div><label style={labelStyle}>Annee</label><input type="number" value={entryYear} onChange={e => setEntryYear(parseInt(e.target.value, 10) || 2025)} style={inputStyle} /></div>
        </div>
...
function HistoryForm({ isOpen, onClose, assets, onAdd }) {
  ...
  async function handleSubmit() {
    if (!assetId || !investment) return;
    setSaving(true);
    const { data, error } = await supabase.from("investments").insert({ asset_id: parseInt(assetId, 10), month, year: entryYear, investment: parseFloat(investment) || 0, buy_price: parseFloat(buyPrice) || 0, quantity: parseFloat(quantity) || 0, stop_loss: 0, take_profit: 0, type: "history" }).select().single();
    setSaving(false);
    if (!error && data) { onAdd({ id: data.id, assetId: data.asset_id, month: data.month, year: data.year, investment: parseFloat(data.investment), buyPrice: parseFloat(data.buy_price), quantity: parseFloat(data.quantity), type: data.type }); setAssetId(""); setInvestment(""); setBuyPrice(""); setQuantity(""); onClose(); }
  }
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="AJOUT HISTORIQUE">
      <div style={{ background: "rgba(247,147,26,0.08)", border: "1px solid rgba(247,147,26,0.2)", borderRadius: 10, padding: "10px 14px", marginBottom: 14, fontSize: 12, color: "rgba(247,147,26,0.9)" }}>Saisie d un investissement deja realise</div>
      <div style={{ display: "grid", gap: 14 }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
          <div><label style={labelStyle}>Actif</label><select value={assetId} onChange={e => setAssetId(e.target.value)} style={{ ...inputStyle, cursor: "pointer" }}><option value="">Choisir...</option>{assets.map(a => <option key={a.id} value={a.id} style={{ background: "#1a1a2e" }}>{a.name} ({a.ticker})</option>)}</select></div>
          <div><label style={labelStyle}>Mois</label><select value={month} onChange={e => setMonth(parseInt(e.target.value, 10))} style={{ ...inputStyle, cursor: "pointer" }}>{MONTHS.map((m, i) => <option key={i} value={i} style={{ background: "#1a1a2e" }}>{m}</option>)}</select></div>
          <div><label style={labelStyle}>Annee</label><input type="number" value={entryYear} onChange={e => setEntryYear(parseInt(e.target.value, 10) || 2024)} style={inputStyle} /></div>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
          <div><label style={labelStyle}>Investissement EUR</label><input type="number" value={investment} onChange={e => setInvestment(e.target.value)} placeholder="100.00" style={inputStyle} /></div>
        ...
        <div style={{ borderTop: "1px solid rgba(255,255,255,0.06)", padding: "16px 20px", animation: "fadeIn 0.3s" }}>
          ...
          {historyEntries.length > 0 && (
            <div style={{ marginBottom: 14 }}>
              <div style={{ fontSize: 11, fontWeight: 600, color: "#f7931a", letterSpacing: 1.5, marginBottom: 8 }}><span style={{ width: 8, height: 8, borderRadius: "50%", background: "#f7931a", display: "inline-block", marginRight: 6 }} />HISTORIQUE</div>
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>{historyEntries.map(e => <EntryRow key={e.id} entry={e} onDelete={onDelete} />)}</div>
            </div>
          )}
          {investEntries.length > 0 && (
            <div>
              <div style={{ fontSize: 11, fontWeight: 600, color: "#00d2ff", letterSpacing: 1.5, marginBottom: 8 }}><span style={{ width: 8, height: 8, borderRadius: "50%", background: "#00d2ff", display: "inline-block", marginRight: 6 }} />INVESTISSEMENTS</div>
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>{investEntries.map(e => <EntryRow key={e.id} entry={e} onDelete={onDelete} />)}</div>
            </div>
          )}
...
function MarketView({ assetData, prices, pricesLoading, lastUpdated, onRefresh, debugLog }) {
  ...
  return (
    <div style={{ animation: "fadeIn 0.3s" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
        <div style={{ fontSize: 11, fontWeight: 600, color: "rgba(255,255,255,0.3)", letterSpacing: 2 }}>PRIX EN TEMPS REEL</div>
      ...
      {debugLog && debugLog.length > 0 && (
        <div style={{ background: "rgba(255,200,0,0.08)", border: "1px solid rgba(255,200,0,0.2)", borderRadius: 10, padding: "10px 14px", marginBottom: 14, fontSize: 11, color: "rgba(255,200,0,0.8)", fontFamily: "monospace", wordBreak: "break-all" }}>
          <div style={{ fontWeight: 700, marginBottom: 4, color: "#ffc800" }}>DEBUG PRIX</div>
          {debugLog.map((line, i) => <div key={i} style={{ marginTop: 2 }}>{line}</div>)}
        </div>
      )}
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {activeAssets.map(asset => {
          ...
          const pnlPct = pnl != null && asset.totalCum > 0 ? (pnl / asset.totalCum) * 100 : null;
          const isGain = pnl != null && pnl >= 0;
          const isStock = STOCK_TICKERS[asset.ticker];
          return (
            <div key={asset.id} style={{ display: "grid", gridTemplateColumns: "200px 1fr 1fr 1fr 1fr 1fr", gap: 12, alignItems: "center", padding: "14px 20px", background: "rgba(255,255,255,0.03)", border: "1px solid " + asset.color + "22", borderRadius: 12 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                ...
                <div>
                  ...
                  {isStock && <div style={{ fontSize: 9, color: "#f7931a", fontWeight: 700 }}>ACTION USD</div>}
                </div>
              </div>
              ...
              <div style={{ textAlign: "center" }}>
                ...
                <div style={{ fontSize: 13, fontWeight: 700, color: pnl != null ? (isGain ? "#2ed573" : "#ff4757") : "rgba(255,255,255,0.3)" }}>{pnl != null ? fmt(pnl) : "\u2014"}</div>
  ...
  return (
    <div style={{ animation: "fadeIn 0.3s" }}>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 24 }}>
        {bestAsset && <div style={{ background: "rgba(46,213,115,0.08)", border: "1px solid rgba(46,213,115,0.2)", borderRadius: 16, padding: "20px 22px" }}><div style={{ fontSize: 10, fontWeight: 600, color: "#2ed573", letterSpacing: 2, marginBottom: 8 }}>MEILLEUR ACTIF</div><div style={{ fontFamily: "Bebas Neue,sans-serif", fontSize: 24, color: "#fff", marginBottom: 4 }}>{bestAsset.name}</div><div style={{ fontSize: 16, fontWeight: 700, color: "#2ed573" }}>{fmtPct(bestAsset.pnlPct)} ({fmt(bestAsset.pnl)})</div></div>}
        {worstAsset && worstAsset.ticker !== (bestAsset && bestAsset.ticker) && <div style={{ background: "rgba(255,71,87,0.08)", border: "1px solid rgba(255,71,87,0.2)", borderRadius: 16, padding: "20px 22px" }}><div style={{ fontSize: 10, fontWeight: 600, color: "#ff4757", letterSpacing: 2, marginBottom: 8 }}>ACTIF EN BAISSE</div><div style={{ fontFamily: "Bebas Neue,sans-serif", fontSize: 24, color: "#fff", marginBottom: 4 }}>{worstAsset.name}</div><div style={{ fontSize: 16, fontWeight: 700, color: "#ff4757" }}>{fmtPct(worstAsset.pnlPct)} ({fmt(worstAsset.pnl)})</div></div>}
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
        <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 16, padding: "20px 24px" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {activeAssets.slice(0, 12).map(asset => { const pct = totalInvested > 0 ? (asset.totalCum / totalInvested) * 100 : 0; return (<div key={asset.id}><div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}><span style={{ fontSize: 12, color: "#fff", fontWeight: 600 }}>{asset.name}</span><span style={{ fontSize: 12, color: asset.color, fontWeight: 700 }}>{pct.toFixed(1)}%</span></div><div style={{ height: 6, background: "rgba(255,255,255,0.06)", borderRadius: 3, overflow: "hidden" }}><div style={{ height: "100%", width: pct + "%", background: "linear-gradient(90deg," + asset.color + "88," + asset.color + ")", borderRadius: 3, transition: "width 0.8s ease" }} /></div></div>); })}
          </div>
        </div>
        <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 16, padding: "20px 24px" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {assetsWithPnl.slice(0, 12).map(asset => { const maxPct = Math.max(...assetsWithPnl.map(a => Math.abs(a.pnlPct || 0)), 1); const barWidth = Math.abs(asset.pnlPct || 0) / maxPct * 100; const isGain = (asset.pnlPct || 0) >= 0; return (<div key={asset.id} style={{ display: "flex", alignItems: "center", gap: 10 }}><span style={{ width: 60, fontSize: 11, fontWeight: 700, color: asset.color, flexShrink: 0 }}>{asset.ticker}</span><div style={{ flex: 1, height: 8, background: "rgba(255,255,255,0.06)", borderRadius: 4, overflow: "hidden" }}><div style={{ height: "100%", width: barWidth + "%", background: isGain ? "linear-gradient(90deg,#2ed57388,#2ed573)" : "linear-gradient(90deg,#ff475788,#ff4757)", borderRadius: 4 }} /></div><span style={{ width: 65, fontSize: 11, fontWeight: 700, color: isGain ? "#2ed573" : "#ff4757", textAlign: "right", flexShrink: 0 }}>{fmtPct(asset.pnlPct)}</span></div>); })}
              ...
              return (
                <div key={asset.id}>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
                    ...
                    <span style={{ fontSize: 12, color: "rgba(255,255,255,0.5)" }}>{fmt(asset.totalCum)} investi{currentValue != null && <span style={{ color: isGain ? "#2ed573" : "#ff4757" }}> / {fmt(currentValue)} actuel</span>}</span>
                  </div>
...
export default function CryptoTracker() {
  ...
  function showToast(msg, type) { setToast({ msg, type: type || "info" }); setTimeout(() => setToast(null), 3000); }

  // ---- fetchStockPrices : Twelve Data + conversion USD->EUR en temps reel ----
  async function fetchStockPrices(tickers) {
    const TWELVE_KEY = "967a2fc9b6dc4e5e82f7630acac1a241";
    const results = {};

    // Etape 1 : taux USD->EUR via Twelve Data (forex)
    let usdToEur = null;
    try {
      const fxUrl = "https://api.twelvedata.com/exchange_rate?symbol=USD/EUR&apikey=" + TWELVE_KEY;
      const fxRes = await fetch(fxUrl);
      const fxData = await fxRes.json();
      const rate = parseFloat(fxData?.rate);
      if (!isNaN(rate) && rate > 0) {
        usdToEur = rate;
        console.log("USD->EUR (Twelve Data):", usdToEur);
      } else {
        console.warn("Twelve Data FX indisponible:", fxData);
      }
    } catch (e) { console.warn("Twelve Data FX error:", e); }

    // Si pas de taux FX, on n affiche rien plutot que d afficher USD comme EUR
    if (!usdToEur) {
      console.warn("Taux USD->EUR indisponible - prix actions non affiches.");
      return results;
    }

    // Etape 2 : prix action via Twelve Data price endpoint
    await Promise.all(tickers.map(async (ticker) => {
      try {
        const url = "https://api.twelvedata.com/price?symbol=" + encodeURIComponent(ticker) + "&apikey=" + TWELVE_KEY;
        ...
        if (!isNaN(priceUsd) && priceUsd > 0) {
          ...
          console.log(ticker + " USD:" + priceUsd.toFixed(4) + " -> EUR:" + priceEur.toFixed(4) + " (x" + usdToEur.toFixed(4) + ")");
        } else {
          console.warn("Twelve Data: pas de prix pour", ticker, data);
        }
      } catch (e) { console.warn("Twelve Data price error:", ticker, e); }
    }));
    return results;
  }

  // ---- fetchPrices : crypto + stocks, avec setPricesLoading correct ----
  async function fetchPrices(assetList) {
    ...
    // CRYPTO
    const cryptoIds = tickers.map(t => COINGECKO_IDS[t]).filter(Boolean);
    let cryptoPrices = {};
    ...
    console.log("ALL PRICES:", merged);
    const debugInfo = Object.entries(merged).map(([t, v]) => t + "=" + (v != null ? (typeof v === "number" ? v.toFixed(4) : JSON.stringify(v)) + "EUR" : "N/A")).join(" | ");
    const stockInfo = " | STOCKS:" + JSON.stringify(Object.fromEntries(Object.entries(stockPrices).map(([k,v]) => [k, typeof v === "number" ? v.toFixed(4) : v])));
    setDebugLog([new Date().toLocaleTimeString() + " -> Crypto:" + Object.keys(cryptoPrices).length + " Stock:" + Object.keys(stockPrices).length, debugInfo + stockInfo]);
  ...
  async function loadAll() {
    ...
    if (invData) { setEntries(invData.map(e => ({ id: e.id, assetId: e.asset_id, month: e.month, year: e.year, investment: parseFloat(e.investment) || 0, buyPrice: parseFloat(e.buy_price) || 0, quantity: parseFloat(e.quantity) || 0, type: e.type || "invest" }))); }
    setLoading(false);
    await fetchPrices(loadedAssets);
  }

  useEffect(() => { loadAll(); }, []);
  useEffect(() => { if (assets.length === 0) return; const interval = setInterval(() => fetchPrices(), 60000); return () => clearInterval(interval); }, [assets]);

  async function handleDeleteEntry(id) { await supabase.from("investments").delete().eq("id", id); setEntries(prev => prev.filter(e => e.id !== id)); }
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0d0d1a", fontFamily: "DM Sans,sans-serif", color: "#fff" }}>
      ...
      `}</style>
      {toast && <div style={{ position: "fixed", top: 20, left: "50%", transform: "translateX(-50%)", zIndex: 2000, padding: "10px 20px", borderRadius: 10, fontSize: 13, fontWeight: 600, background: toast.type === "success" ? "#2ed573" : "#00d2ff", color: "#000", animation: "toastIn 0.3s ease", whiteSpace: "nowrap" }}>{toast.msg}</div>}
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 14, flexWrap: "wrap" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
            ...
            <div style={{ display: "flex", background: "rgba(255,255,255,0.05)", borderRadius: 10, overflow: "hidden" }}>
              {VIEWS.map(v => <button key={v.k} onClick={() => setActiveView(v.k)} style={{ padding: "7px 14px", border: "none", cursor: "pointer", fontSize: 12, fontWeight: 600, background: activeView === v.k ? "rgba(0,210,255,0.15)" : "transparent", color: activeView === v.k ? "#00d2ff" : "rgba(255,255,255,0.4)", display: "flex", alignItems: "center", gap: 5 }}>{v.icon && <span style={{ opacity: 0.7 }}>{v.icon}</span>}{v.l}</button>)}
            </div>
            <button onClick={() => setShowHistory(true)} style={{ padding: "7px 12px", border: "1px solid rgba(247,147,26,0.3)", borderRadius: 10, background: "rgba(247,147,26,0.08)", color: "#f7931a", cursor: "pointer", fontSize: 12, fontWeight: 600, display: "flex", alignItems: "center", gap: 5 }}><SvgHistory /> Historique</button>
            <button onClick={() => setShowAsset(true)} style={{ padding: "7px 12px", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 10, background: "rgba(255,255,255,0.04)", color: "rgba(255,255,255,0.6)", cursor: "pointer", fontSize: 12, fontWeight: 600, display: "flex", alignItems: "center", gap: 5 }}><SvgPlus /> Actif</button>
            <button onClick={() => setShowInvest(true)} style={{ padding: "7px 16px", background: "linear-gradient(135deg,#00d2ff,#3a7bd5)", border: "none", borderRadius: 10, color: "#fff", cursor: "pointer", fontSize: 12, fontWeight: 700, display: "flex", alignItems: "center", gap: 5, boxShadow: "0 4px 16px rgba(0,210,255,0.2)" }}><SvgPlus /> Investir</button>
          </div>
        </div>
      </header>
          ...
          <>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 14, marginBottom: 24 }}>
              {[
                ...
                { label: "VALEUR PORTEFEUILLE", value: fmt(totalCurrentValue), sub: "temps reel", accent: "#14F195", gradient: "linear-gradient(135deg,#14F19522,#0d996622)" },
                { label: "PNL GLOBAL", value: totalPnl != null ? fmt(totalPnl) : "\u2014", sub: totalPnl != null ? (totalPnl >= 0 ? "benefice" : "perte") : "\u2014", accent: totalPnl != null ? (totalPnl >= 0 ? "#2ed573" : "#ff4757") : "#666", gradient: totalPnl != null ? (totalPnl >= 0 ? "linear-gradient(135deg,#2ed57322,#17a85922)" : "linear-gradient(135deg,#ff475722,#c0392b22)") : "linear-gradient(135deg,#66666622,#33333322)" },
                { label: "ACTIFS", value: String(assetData.filter(a => a.totalCum > 0).length), sub: "sur " + assets.length + " suivis", accent: "#f7931a", gradient: "linear-gradient(135deg,#f7931a22,#c9711522)" },
              ].map((kpi, i) => (
            ...
            <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 16, padding: "18px 22px", marginBottom: 22 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(12,1fr)", gap: 6 }}>
                {totalMonthly.map((val, i) => (
                  <div key={i} style={{ textAlign: "center" }}>
                    <div style={{ height: 70, display: "flex", alignItems: "flex-end", justifyContent: "center", marginBottom: 6 }}><div style={{ width: "80%", height: Math.max(4, (val / maxMonthly) * 70), background: val > 0 ? "linear-gradient(to top,#00d2ff44,#00d2ff)" : "rgba(255,255,255,0.04)", borderRadius: 4 }} /></div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {assetData.map((asset, i) => <div key={asset.id} style={{ animation: "fadeSlideUp 0.4s ease " + (0.3 + i * 0.04) + "s both" }}><AssetCard asset={asset} year={year} onDelete={handleDeleteEntry} prices={prices} /></div>)}
            </div>
          </>
        )}
        {activeView === "market" && <MarketView assetData={assetData} prices={prices} pricesLoading={pricesLoading} lastUpdated={lastUpdated} onRefresh={() => fetchPrices()} debugLog={debugLog} />}
                  ...
                  {assetData.map(asset => (
                    <tr key={asset.id} style={{ borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                      <td style={{ padding: "12px 20px", position: "sticky", left: 0, backgroundColor: "#0d0d1a", borderRight: "1px solid rgba(255,255,255,0.06)" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 8 }}><div style={{ width: 8, height: 8, borderRadius: "50%", background: asset.color }} /><span style={{ fontFamily: "Bebas Neue,sans-serif", fontSize: 14, letterSpacing: 1.5, color: "#fff" }}>{asset.name}</span>{asset.objective && <span style={{ fontSize: 9, fontWeight: 700, padding: "1px 5px", borderRadius: 4, background: asset.color + "22", color: asset.color }}>{asset.objective}</span>}</div>
                      </td>
                      {asset.monthlyData.map((val, mi) => <td key={mi} style={{ padding: "12px 10px", textAlign: "center", color: val > 0 ? "#fff" : "rgba(255,255,255,0.12)", fontSize: 12, background: val > 0 ? asset.color + "08" : "transparent" }}>{val > 0 ? fmt(val) : "\u2014"}</td>)}
