...
export default function CryptoTracker() {
  ...
  // ---- fetchStockPrices : Yahoo Finance (pas de cle API, pas de limite) ----
  async function fetchStockPrices(tickers) {
    const results = {};
    await Promise.all(tickers.map(async (ticker) => {
      try {
        // Yahoo Finance: retourne EUR directement avec le symbole EURUSD=X en meme appel
        const symbols = ticker + "," + "EURUSD=X";
        ...
        spark.forEach(item => {
          ...
          const close = item.response?.[0]?.meta?.regularMarketPrice;
          if (sym === ticker && close != null) priceUsd = parseFloat(close);
          if (sym === "EURUSD=X" && close != null) usdToEur = parseFloat(close);
        });

        if (priceUsd && usdToEur && usdToEur > 0) {
          const priceEur = priceUsd * usdToEur;
          results[ticker] = { usd: priceUsd, eur: priceEur };
          console.log(ticker + " USD:" + priceUsd + " EUR:" + priceEur + " (rate:" + usdToEur + ")");
        } else {
          console.warn("Yahoo: donnees manquantes pour", ticker, { priceUsd, usdToEur });
        }
      } catch (e) { console.warn("Yahoo Finance error:", ticker, e); }
    }));
    return results;
  }
