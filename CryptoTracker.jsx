    ...
    <div style={{ position: "fixed", inset: 0, zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
      ...
      <div style={{ position: "relative", zIndex: 1, background: "#1a1a2e", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 16, padding: 24, width: "100%", maxWidth: 560, maxHeight: "85vh", overflowY: "auto", boxShadow: "0 24px 80px rgba(0,0,0,0.5)", animation: "modalIn 0.25s ease" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
          ...
          <button onClick={onClose} style={{ background: "none", border: "none", color: "rgba(255,255,255,0.4)", fontSize: 24, cursor: "pointer", lineHeight: 1 }}>&times;</button>
        </div>
        {children}
      </div>
    </div>
    ...
    <Modal isOpen={isOpen} onClose={onClose} title="NOUVEL INVESTISSEMENT">
      <div style={{ display: "grid", gap: 14 }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
          ...
          <div><label style={labelStyle}>Ann&eacute;e</label>
            <input type="number" value={entryYear} onChange={e => setEntryYear(parseInt(e.target.value, 10) || 2025)} style={inputStyle}/>
          </div>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
          <div><label style={labelStyle}>Investissement (&euro;)</label><input type="number" value={investment} onChange={e => setInvestment(e.target.value)} placeholder="100.00" style={inputStyle}/></div>
          <div><label style={labelStyle}>Prix d&apos;achat (&euro;)</label><input type="number" value={buyPrice} onChange={e => setBuyPrice(e.target.value)} placeholder="0.00" style={inputStyle} step="any"/></div>
        </div>
        <div><label style={labelStyle}>Quantit&eacute; (jetons)</label><input type="number" value={quantity} onChange={e => setQuantity(e.target.value)} placeholder="0" style={inputStyle} step="any"/></div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
          <div><label style={labelStyle}>Stop-Loss (&euro;)</label><input type="number" value={stopLoss} onChange={e => setStopLoss(e.target.value)} placeholder="0.00" style={inputStyle} step="any"/></div>
          <div><label style={labelStyle}>Take-Profit (&euro;)</label><input type="number" value={takeProfit} onChange={e => setTakeProfit(e.target.value)} placeholder="0.00" style={inputStyle} step="any"/></div>
        </div>
    ...
    <div style={{ background: "rgba(255,255,255,0.03)", border: `1px solid ${asset.color}22`, borderRadius: 14, overflow: "hidden", transition: "all 0.3s" }}
      ...
      <div onClick={() => setExpanded(!expanded)} style={{ display: "flex", alignItems: "center", gap: 14, padding: "16px 20px", cursor: "pointer", background: expanded ? "rgba(255,255,255,0.02)" : "transparent" }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: 12, color: "rgba(255,255,255,0.4)", marginTop: 2 }}>{asset.entriesCum.length} invest. &bull; {fmt(totalInv)}</div>
        </div>
      ...
      {expanded && (
        <div style={{ borderTop: "1px solid rgba(255,255,255,0.06)", padding: "16px 20px", animation: "fadeIn 0.3s" }}>
          ...
          <div style={{ fontSize: 11, fontWeight: 600, color: "rgba(255,255,255,0.3)", letterSpacing: 1.5, marginBottom: 8 }}>HISTORIQUE (cumul &le; {year})</div>
