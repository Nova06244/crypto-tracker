...
function SvgChart() {
  return (
    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" style={{ display: "inline", verticalAlign: "middle", marginRight: 6 }}>
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="NOUVEL INVESTISSEMENT">
      <div style={{ display: "grid", gap: 14 }}>
        ...
        <button onClick={handleSubmit} disabled={saving} style={{ marginTop: 4, padding: "12px 0", width: "100%", background: saving ? "rgba(255,255,255,0.1)" : "linear-gradient(135deg,#00d2ff,#3a7bd5)", border: "none", borderRadius: 10, color: "#fff", fontSize: 14, fontWeight: 700, letterSpacing: 1.5, cursor: saving ? "not-allowed" : "pointer", textTransform: "uppercase", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
          {saving ? <SvgSpinner /> : null} {saving ? "Enregistrement..." : "Ajouter"}
        </button>
      </div>
    </Modal>
  );
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="AJOUT HISTORIQUE">
      ...
      <div style={{ display: "grid", gap: 14 }}>
        ...
        <div><label style={labelStyle}>Quantite obtenue</label><input type="number" value={quantity} onChange={e => setQuantity(e.target.value)} placeholder="0" style={inputStyle} step="any" /></div>
        <button onClick={handleSubmit} disabled={saving} style={{ marginTop: 4, padding: "12px 0", width: "100%", background: saving ? "rgba(255,255,255,0.1)" : "linear-gradient(135deg,#f7931a,#e67e22)", border: "none", borderRadius: 10, color: "#fff", fontSize: 14, fontWeight: 700, letterSpacing: 1.5, cursor: saving ? "not-allowed" : "pointer", textTransform: "uppercase", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
          {saving ? <SvgSpinner /> : null} {saving ? "Enregistrement..." : "Enregistrer"}
        </button>
      </div>
    </Modal>
  );
...
function AddAssetModal({ isOpen, onClose, onAdd }) {
  ...
  async function handleSubmit() {
    ...
    setSaving(false);
    if (!error && data) {
      onAdd({ id: data.id, name: data.name, ticker: data.ticker, objective: data.objective, color: data.color });
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="AJOUTER UN ACTIF">
      <div style={{ display: "grid", gap: 14 }}>
        ...
        <button onClick={handleSubmit} disabled={saving} style={{ marginTop: 4, padding: "12px 0", width: "100%", background: saving ? "rgba(255,255,255,0.1)" : "linear-gradient(135deg,#14F195,#3a7bd5)", border: "none", borderRadius: 10, color: "#fff", fontSize: 14, fontWeight: 700, letterSpacing: 1.5, cursor: saving ? "not-allowed" : "pointer", textTransform: "uppercase" }}>
...
function EntryRow({ entry, onDelete }) {
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "90px 1fr 1fr 1fr 36px", gap: 8, alignItems: "center", padding: "8px 12px", borderRadius: 8, background: isHistory ? "rgba(247,147,26,0.06)" : "rgba(0,210,255,0.04)", border: "1px solid " + (isHistory ? "rgba(247,147,26,0.15)" : "rgba(0,210,255,0.12)"), fontSize: 13 }}>
      ...
      <span style={{ color: "rgba(255,255,255,0.5)" }}>{entry.buyPrice > 0 ? "@ " + fmt(entry.buyPrice) : "\u2014"}</span>
      ...
      <div onClick={() => setExpanded(!expanded)} style={{ display: "flex", alignItems: "center", gap: 14, padding: "16px 20px", cursor: "pointer", background: expanded ? "rgba(255,255,255,0.02)" : "transparent" }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: 12, color: "rgba(255,255,255,0.4)", marginTop: 2, display: "flex", gap: 10 }}>
            ...
            <span>{fmt(asset.totalCum)}</span>
          </div>
        </div>
        <div style={{ textAlign: "right" }}>
          <div style={{ fontFamily: "Bebas Neue,sans-serif", fontSize: 20, color: "#fff" }}>{fmt(asset.totalCum)}</div>
          <div style={{ fontSize: 11, color: "rgba(255,255,255,0.35)" }}>{asset.qtyCum > 0 ? asset.qtyCum.toLocaleString("fr-FR") + " jetons" : "\u2014"}</div>
        </div>
      ...
      {expanded && (
        <div style={{ borderTop: "1px solid rgba(255,255,255,0.06)", padding: "16px 20px", animation: "fadeIn 0.3s" }}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 10, marginBottom: 16 }}>
            {[
              { label: "PRU moyen", value: asset.avgBuyCum > 0 ? fmt(asset.avgBuyCum) : "\u2014" },
              { label: "Quantite totale", value: asset.qtyCum > 0 ? asset.qtyCum.toLocaleString("fr-FR") : "\u2014" },
              { label: "Total investi", value: fmt(asset.totalCum) },
            ].map((s, i) => (
...
export default function CryptoTracker() {
  ...
  async function loadAll() {
    ...
    if (assetsData && assetsData.length > 0) {
      setAssets(assetsData.map(a => ({ id: a.id, name: a.name, ticker: a.ticker, objective: a.objective || "", color: a.color || "#00d2ff" })));
    } else {
      const { data: seeded } = await supabase.from("assets").insert(DEFAULT_ASSETS).select();
      if (seeded) setAssets(seeded.map(a => ({ id: a.id, name: a.name, ticker: a.ticker, objective: a.objective || "", color: a.color || "#00d2ff" })));
    }

    if (invData) {
      setEntries(invData.map(e => ({
        id: e.id,
        assetId: e.asset_id,
        month: e.month,
        year: e.year,
        investment: parseFloat(e.investment) || 0,
        buyPrice: parseFloat(e.buy_price) || 0,
  ...
  const assetData = useMemo(() => {
    return assets.map(asset => {
      ...
      const avgBuyCum = totalCum > 0
        ? entriesCum.reduce((s, e) => s + (e.buyPrice || 0) * (e.investment || 0), 0) / totalCum : 0;
  ...
  const kpis = [
    ...
    { label: "ACTIFS", value: String(activeCount), sub: "sur " + assets.length + " suivis", accent: "#14F195", gradient: "linear-gradient(135deg,#14F19522,#0d996622)" },
    { label: "INVESTISSEMENTS", value: String(entries.filter(e => e.type === "invest").length), sub: "nouveaux", accent: "#00d2ff", gradient: "linear-gradient(135deg,#00d2ff22,#3a7bd522)" },
    { label: "HISTORIQUE", value: String(entries.filter(e => e.type === "history").length), sub: "entrees passees", accent: "#f7931a", gradient: "linear-gradient(135deg,#f7931a22,#c9711522)" },
  ];
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0d0d1a", fontFamily: "DM Sans,sans-serif", color: "#fff", position: "relative" }}>
      ...
      <header style={{ padding: "24px 32px 20px", borderBottom: "1px solid rgba(255,255,255,0.06)", background: "rgba(13,13,26,0.9)", backdropFilter: "blur(20px)", position: "sticky", top: 0, zIndex: 100 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 14, flexWrap: "wrap" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
            <div style={{ width: 42, height: 42, borderRadius: 12, background: "linear-gradient(135deg,#00d2ff,#3a7bd5)", display: "flex", alignItems: "center", justifyContent: "center" }}>
              <SvgWallet />
            </div>
            <div>
              <h1 style={{ fontFamily: "Bebas Neue,sans-serif", fontSize: 28, letterSpacing: 4, margin: 0, lineHeight: 1, color: "#fff" }}>INVESTISSEMENT CRYPTO</h1>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 4, background: "rgba(255,255,255,0.05)", borderRadius: 10, padding: "6px 4px" }}>
              <button onClick={() => setYear(y => y - 1)} style={{ background: "none", border: "none", cursor: "pointer", color: "rgba(255,255,255,0.4)", padding: "4px 8px", borderRadius: 6, display: "flex", alignItems: "center" }}><SvgLeft /></button>
              <span style={{ fontFamily: "Bebas Neue,sans-serif", fontSize: 20, letterSpacing: 2, color: "#fff", minWidth: 60, textAlign: "center" }}>{year}</span>
              <button onClick={() => setYear(y => y + 1)} style={{ background: "none", border: "none", cursor: "pointer", color: "rgba(255,255,255,0.4)", padding: "4px 8px", borderRadius: 6, display: "flex", alignItems: "center" }}><SvgRight /></button>
            </div>
            <div style={{ display: "flex", background: "rgba(255,255,255,0.05)", borderRadius: 10, overflow: "hidden" }}>
              {[{ k: "dashboard", l: "Dashboard" }, { k: "table", l: "Tableau" }].map(v => (
                <button key={v.k} onClick={() => setActiveView(v.k)} style={{ padding: "8px 16px", border: "none", cursor: "pointer", fontSize: 12, fontWeight: 600, background: activeView === v.k ? "rgba(0,210,255,0.15)" : "transparent", color: activeView === v.k ? "#00d2ff" : "rgba(255,255,255,0.4)" }}>{v.l}</button>
              ))}
            </div>
            <button onClick={() => setShowHistory(true)} style={{ padding: "8px 14px", border: "1px solid rgba(247,147,26,0.3)", borderRadius: 10, background: "rgba(247,147,26,0.08)", color: "#f7931a", cursor: "pointer", fontSize: 12, fontWeight: 600, display: "flex", alignItems: "center", gap: 6 }}>
              <SvgHistory /> Historique
            </button>
            <button onClick={() => setShowAsset(true)} style={{ padding: "8px 14px", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 10, background: "rgba(255,255,255,0.04)", color: "rgba(255,255,255,0.6)", cursor: "pointer", fontSize: 12, fontWeight: 600, display: "flex", alignItems: "center", gap: 6 }}>
              <SvgPlus /> Actif
            </button>
            <button onClick={() => setShowInvest(true)} style={{ padding: "8px 18px", background: "linear-gradient(135deg,#00d2ff,#3a7bd5)", border: "none", borderRadius: 10, color: "#fff", cursor: "pointer", fontSize: 12, fontWeight: 700, letterSpacing: 1, display: "flex", alignItems: "center", gap: 6, boxShadow: "0 4px 16px rgba(0,210,255,0.2)" }}>
              <SvgPlus /> Investir
            </button>
          </div>
        </div>
        ...
        {activeView === "dashboard" ? (
          <>
            <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 16, padding: "20px 24px", marginBottom: 28 }}>
              <div style={{ fontSize: 11, fontWeight: 600, color: "rgba(255,255,255,0.4)", letterSpacing: 2, marginBottom: 16 }}><SvgChart /> REPARTITION MENSUELLE {year}</div>
                ...
                {totalMonthly.map((val, i) => (
                  <div key={i} style={{ textAlign: "center" }}>
                    <div style={{ height: 80, display: "flex", alignItems: "flex-end", justifyContent: "center", marginBottom: 8 }}>
                      <div style={{ width: "100%", maxWidth: 40, height: Math.max(4, (val / maxMonthly) * 80), background: val > 0 ? "linear-gradient(to top,#00d2ff44,#00d2ff)" : "rgba(255,255,255,0.04)", borderRadius: 6 }} />
                    </div>
                    <div style={{ fontSize: 10, fontWeight: 600, color: val > 0 ? "#00d2ff" : "rgba(255,255,255,0.2)" }}>{MONTHS[i]}</div>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                <thead>
                  <tr>
                    ...
                    {MONTHS.map((m, i) => <th key={i} style={{ padding: "14px 12px", textAlign: "center", background: "rgba(255,255,255,0.04)", borderBottom: "1px solid rgba(255,255,255,0.08)", color: "rgba(255,255,255,0.5)", fontSize: 10, fontWeight: 600, minWidth: 80 }}>{m}</th>)}
                    <th style={{ padding: "14px 16px", textAlign: "center", background: "rgba(0,210,255,0.06)", borderBottom: "1px solid rgba(255,255,255,0.08)", color: "#00d2ff", fontSize: 10, fontWeight: 700 }}>TOTAL &lt;= {year}</th>
                  </tr>
                </thead>
                <tbody>
                  {assetData.map(asset => (
                    <tr key={asset.id} style={{ borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                      ...
                      {asset.monthlyData.map((val, mi) => (
                        <td key={mi} style={{ padding: "12px", textAlign: "center", color: val > 0 ? "#fff" : "rgba(255,255,255,0.12)", fontSize: 12, background: val > 0 ? asset.color + "08" : "transparent" }}>{val > 0 ? fmt(val) : "\u2014"}</td>
                      ))}
